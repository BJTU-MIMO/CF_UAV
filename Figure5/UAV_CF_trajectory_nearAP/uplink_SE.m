...
[SE_CF0] = functionUplinkSE_UAV_CF(R,Q,HMean,L,(1-partial)*P_EH_CF0,kappa,noiseVariance,tauc,taue,taup);
P_pilot = partial*P_EH_CF0;
destination = (0.85 + 1i*0.85)*100;  % UAV destination

allAP = [APpositions(1:L);destination];
...
for n=1:slot
    ...
    minDistance = 100*sqrt(2);
    allAngle = zeros(1,L+1);
    allDistance = zeros(1,L+1);
    for r=1:(L+1)
        allAngle(r) = angle(allAP(r)-UAVposition_initial);
        allDistance(r) = abs(allAP(r)-UAVposition_initial);
        aa = real(destination)-real(allAP(r));
        bb = imag(destination)-imag(allAP(r));
        
        if allAngle(r)>=0 && allAngle(r)<=pi/2 && allDistance(r)<minDistance && allDistance(r)>=interval && aa>=0 && bb>=0
            theta = allAngle(r);
            minDistance = allDistance(r);
        end
    end
    ...
    for s = 1:length(theta)
        ...
        [SE_CF] = functionUplinkSE_UAV_CF(R,Q,HMean,L,(1-partial)*P_EH_CF,kappa,noiseVariance,tauc,taue,taup);
    ...
    P_pilot = partial*P_EH_CF_angle(index);
...
%%
figure;
plot(sort([SE_CF0,SE_trajectory(1:time)]),linspace(0,1,time+1),'r-','LineWidth',2);
