...
[P_EH_SC0] = functionDownlinkEH_UAV_SC(R,Q,HMean,L,pd,kappa,tauc,taue);
[SE_SC0] = functionUplinkSE_UAV_SC(Hhat,C,L,N,(1-partial)*P_EH_SC0,kappa,noiseVariance,nbrOfRealizations,tauc,taue,taup);

P_pilot = partial*P_EH_SC0;
...
for n=1:slot
    ...
    for s = 1:length(theta)
        ...
        [P_EH_SC] = functionDownlinkEH_UAV_SC(R,Q,HMean,L,pd,kappa,tauc,taue);
        [SE_SC] = functionUplinkSE_UAV_SC(Hhat,C,L,N,(1-partial)*P_EH_SC,kappa,noiseVariance,nbrOfRealizations,tauc,taue,taup);
    ...
    P_pilot = partial*P_EH_SC_angle(index);
...
%%
figure;
plot(sort([SE_SC0,SE_trajectory(1:time)]),linspace(0,1,time+1),'r-','LineWidth',2);
