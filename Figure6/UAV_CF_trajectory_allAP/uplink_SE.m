...
for r = 1:setups
    ...
    allAP = [sort(APpositions(1:L));destination];
    ...
    for n=1:slot
        ...
        if abs(destination-UAVposition_initial) < interval && x == L+1
            break
        end
        
        
        if abs(allAP(x)-UAVposition_initial) < interval
           x = x + 1; 
        else
           theta = angle(allAP(x)-UAVposition_initial);
        end
...
plot([real(initial),real(UAVposition_trajectory(1:time))],[imag(initial),imag(UAVposition_trajectory(1:time))],'b-','LineWidth',2);

xlabel('X','Interpreter','latex');
ylabel('Y','Interpreter','latex');
legend('Initial position','Destination position','AP positions','All APs path','Interpreter','latex');
