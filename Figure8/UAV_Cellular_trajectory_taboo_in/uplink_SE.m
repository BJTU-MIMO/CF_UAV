...
for r = 1:setups
    ...
    [P_EH_Cellular0] = functionDownlinkEH_UAV_Cellular(R,Q,G_te,HMean,L,pd,kappa,tauc,taue);
    [SE_Cellular0] = functionUplinkSE_UAV_Cellular(Hhat,C,R_te,L,N,(tauc/(tauc-taup-taue))*(1-partial)*P_EH_Cellular0,kappa,noiseVariance,nbrOfRealizations,tauc,taue,taup,pt);
    P_pilot = (tauc/taup)*partial*P_EH_Cellular0;
    ...
    for n=1:slot
        ...
        for s = 1:length(theta)
            ...
            [P_EH_Cellular] = functionDownlinkEH_UAV_Cellular(R,Q,G_te,HMean,L,pd,kappa,tauc,taue);
            [SE_Cellular] = functionUplinkSE_UAV_Cellular(Hhat,C,R_te,L,N,(tauc/(tauc-taup-taue))*(1-partial)*P_EH_Cellular,kappa,noiseVariance,nbrOfRealizations,tauc,taue,taup,pt);
        ...
        P_pilot = (tauc/taup)*partial*P_EH_Cellular_angle(index);
    ...
    times = times + time +1;
    SE_setups = [SE_setups,SE_Cellular0,SE_trajectory(1:time)];
end
...
plot([real(initial),real(UAVposition_trajectory(1:time))],[imag(initial),imag(UAVposition_trajectory(1:time))],'b-*','LineWidth',0.1);
