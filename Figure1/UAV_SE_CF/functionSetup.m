function [R,R_te,HMean,H,HMeanx,H_Rayleigh,channelGain] = functionSetup(L,N,ASD,nbrOfRealizations)
...
%% Go through all setups
    ...
    verticalDistance=UEheigth-APheigth;
       ...
       distancetoUE_te = abs(APpositions - UEpositions_te);
        ...
        %gainOverNoisedB_te =beta_0 - 10*log10(distancetoUE_te.^2) ;%- noiseVariancedBm; %需要修改 非自由空间损耗 （三段式）
        gainOverNoisedB_te = pathloss_threeslope(distancetoUE_te);
        channelGain_te=db2pow(gainOverNoisedB_te);
        ...
        for l = 1:L
            ...
            angletoUE_te = angle(UEpositions_te-APpositions(l));
            correlationMatrix_te = functionRlocalscattering(N,angletoUE_te,ASDdeg,antennaSpacing);
            R_te(:,:,l) = channelGain_te(l)*correlationMatrix_te;
%             Psi_te(:,:,l) = (p_te*R_te(:,:,l)+sigma2*eye(N))^(-1);
%             G_te(:,:,l) = p_te*R_te(:,:,l)*Psi_te(:,:,l)*R_te(:,:,l);
